...
static string BuildHtml(string title, string content, string side, string footer, string? ogurl = null, string? ogcontent = null)
{
    ...
    if (ogurl != null)
    {
        var type = (ogurl == "https://itoyokan.dev") ? "website" : "article";
        ogcontent = ogcontent ?? "";
        og = @$"
...
<body>
    ...
    <div id=""wrapper"">
        <a href=""https://itoyokan.dev/""><div id=""header""></div></a>
...
var articles = Directory.EnumerateFiles(inputDir)
    ...
    .Select(x =>
    {
        ...
        var bodyTitle = $"<h1><a href=\"https://itoyokan.dev/{yyyy}/{mm}/{dd_no}.html\">{title}</a></h1>";
...
<b>Yohei Ito</b><br />
<br />
GitHub:<a href=""https://github.com/itoyokandev/"">itoyokandev</a>
</div>
<h3>Archive</h3>
<div class=""side_body"">
<ul>
...
await Parallel.ForEachAsync(articles.GroupBy(x => x.Url.yyyy), async (yyyy, _) =>
{
    ...
    foreach (var mm in yyyy.GroupBy(x => x.Url.mm))
    {
        ...
        foreach (var item in mm)
        {
            ...
            var html = BuildHtml("itoyokandev - " + item.Title, item.Body, side, footer, $"https://itoyokan.dev/{filePath}", item.OriginalBody);
...
async Task GenerateIndexWithPagingAsync(IEnumerable<Article> source, string root, string? title)
{
    ...
    var urlRoot = pageRoot == "" ? "https://itoyokan.dev" : ("https://itoyokan.dev/" + pageRoot);
    var page = 1;
    ...
    foreach (var items in articles)
    {
        ...
        var t = (title == null) ? "itoyokandev" : ("itoyokandev - " + title);
        var og = (title == null && page == 1) ? "https://itoyokan.dev" : null;
...
public record PageUrl(string yyyy, string mm, string dd, string dd_no)
{
    ...
    {
        return $"https://itoyokan.dev/{yyyy}/{mm}/{dd_no}.html";
    }
}
